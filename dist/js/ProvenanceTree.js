...
var ProvenanceTree_exports = {};
__export(ProvenanceTree_exports, {
  default: () => ProvenanceTree_default
});
module.exports = __toCommonJS(ProvenanceTree_exports);
...
function ProvenanceTree(d3, selector, _options) {
  ...
  const options = {
    ...
    propertyMap: {},
    initParentKey: import_DataConverter.default.KEY_P_ACT,
    propertyPrefixClear: "",
  ...
  function init() {
    import_jquery.default.extend(options, _options);
    initImageMap();
  ...
  function handleZoomActivation() {
    ...
    (0, import_jquery.default)(selector).click((e) => {
      enableZoom();
    });
    (0, import_jquery.default)(document).on("scroll", (e) => {
      if (!isElementInViewport((0, import_jquery.default)(selector)[0])) {
        disableZoom();
      }
    });
  }
  function enableZoom() {
    ...
    (0, import_jquery.default)(selector).addClass(classNames.hasZoom);
    createZoom();
  }
  function disableZoom() {
    options.zoomActivated = false;
    (0, import_jquery.default)(selector).removeClass(classNames.hasZoom);
    createZoom();
  }
  ...
  function clearCanvas() {
    ...
    if (options.initParentKey === import_DataConverter.default.KEY_P_ENTITY) {
  ...
  function fetchImage(d, ops) {
    fetch(image(d)).then((r) => r.text()).then((svgXml) => {
      ...
      (0, import_jquery.default)("#image--".concat(id, "--").concat(ops.className)).attr("href", toBase64(svgXml));
  ...
  function getImageHeight(d) {
    ...
    const sz2 = getDefaultSize();
    return actions ? actions.height || sz2 : sz2;
  }
  function getImageWidth(d) {
    const actions = getImageActions(d);
    const sz2 = getDefaultSize();
    return actions ? actions.width || sz2 : sz2;
  }
  ...
  function propertyValueSearch(d, imagesForLabel, dict) {
    ...
    if (imagesForLabel) {
      ...
      for (i = 0; i < imagesForLabel.length; i++) {
        ...
        if (subType === label && (!property || properties[property] !== void 0) && (!value || properties[property] === value)) {
  ...
  function getFillColor(d, ops) {
    ...
    const id = getNodeId(d);
    if (actions) {
      const colors2 = options.theme.colors;
      const glowColor = isHighlighted(d) ? colors2.glow.highlighted : colors2.glow.regular;
      fillColor = ops.className === "glow" ? glowColor : actions.color || fillColor;
      const $node = (0, import_jquery.default)("".concat(selector, " #node--").concat(id));
      if (!actions.showMain) {
  ...
  function buildNodes() {
    ...
    $el.nodeEnter = $el.node.enter().append("g").attr("id", (d) => "node--".concat(getNodeId(d))).attr("data-node", (d) => {
      ...
      (0, import_jquery.default)("".concat(selector, " .").concat(classNames.infoCloseBtn)).fadeIn();
    }).call(drag());
    ...
    $el.node.attr("class", (d) => {
      const classNames2 = "node node--".concat(getNodeCat(d), " ").concat(getHoverClass(d)).concat(getHighlightClass(d), " ").concat(d.data.className || "", " ").concat(d.wasClicked || options.visitedNodes.has(getNodeId(d)) ? "is-visited" : "");
      let customClassNames = runCallback("onNodeCssClass", { node: d });
      return classNames2.trim() + " " + customClassNames || "";
    ...
    (0, import_jquery.default)("".concat(selector, " .").concat(classNames.nodes.image)).remove();
  ...
  function appendInfoPanel() {
    ...
    $infoParent = (0, import_jquery.default)(selector).find(".".concat(classNames.info));
    ...
    (0, import_jquery.default)(selector).on("click", ".".concat(classNames.infoCloseBtn), (e) => {
      e.stopPropagation();
      clearInfo();
      runCallback("onInfoCloseClick", { event: e });
      (0, import_jquery.default)(e.currentTarget).hide();
    });
    $el.info = $el.info.append("div").attr("class", classNames.infoMain);
    $info = (0, import_jquery.default)(selector).find(".".concat(classNames.infoMain));
  }
  ...
  function appendInfoElement(d, cls, isNode, property, value) {
    var _a, _b;
    const isNavigation = (_a = options.idNavigate) == null ? void 0 : _a.props[property];
    value = value ? value.toString().replaceAll('"', "") : value;
    let formattedUrl = false;
    let href = "#";
    if (isNavigation && (((_b = options.idNavigate) == null ? void 0 : _b.url) || isNavigation.url) && !isNavigation.callback) {
    ...
    if (!value) {
      elem.style("background-color", function(d2) {
      ...
      }).style("border-color", function(d2) {
  ...
  function initSimulation() {
    ...
    simulation = d3.forceSimulation(data.nodes).alpha(0.5).force("charge", d3.forceManyBody().strength(1)).force("center", d3.forceCenter(getPos(), getPos("onCenterY", "clientHeight")));
  }
  function updateSimulation() {
    simulation.nodes(data.nodes);
    simulation.alpha(0.5).alphaTarget(0).restart();
  }
  function stratify(data2, parentKey) {
    parentKey = parentKey || import_DataConverter.default.KEY_P_ACT;
    let has = {};
    let x = 0;
  ...
  function loadData(data2) {
    if (data2.stratify) {
      ...
      return stratify(data2.stratify);
    } else {
      dataKey = data2.root ? models.root : null;
      const root = data2.root || data2;
      return { root };
    }
  }
  function resolveDataMethod(data2, dataKey2, parentKey) {
    if (dataKey2 === models.stratify) {
      buildTree(stratify(data2, parentKey));
    } else {
      buildTree({ root: data2 });
    }
  }
  function toggleEdgeLabels() {
    (0, import_jquery.default)(selector).toggleClass(classNames.links.hidden);
  }
  ...
  function setUpSvg() {
    ...
    sz.width = (0, import_jquery.default)(selector).width() - margin.left;
    sz.height = (0, import_jquery.default)(selector).height() - margin.top;
    if (sizes) {
    ...
    $el.svg.append("defs").append("marker").attr("id", "arrowhead").attr("viewBox", "-0 -5 10 10").attr("refX", 30).attr("refY", 0).attr("orient", "auto").attr("markerWidth", 8).attr("markerHeight", 8).attr("xoverflow", "visible").append("svg:path").attr("d", "M 0,-5 L 10 ,0 L 0,5").attr("fill", "#999").style("stroke", "none");
...
var ProvenanceTree_default = ProvenanceTree;
